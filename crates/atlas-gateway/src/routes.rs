...
async fn set_cordon(s: &AppState, actor: &Actor, id: &str, cordoned: bool) -> AppResult<Json<Value>> {
    crate::auth::require_role(s.config.auth_required, actor, crate::auth::ROLE_ADMIN)?;
...
async fn get_maintenance(State(s): State<AppState>) -> AppResult<Json<Value>> {
