...
## ⏭ Slice 2 — Write path & job engine (next)

The `StorageDriver` write methods already exist as `NotImplemented` stubs, so this is additive.

- Async **job engine** (model on `guestkit`'s Redis-Streams + worker pattern) with the PDF §10.5
  state machine (`pending→queued→running→verifying→succeeded|failed→rollback`).
- `POST /volumes` (+ `expand`, `delete`) → create a PVC/RBD image; `202 Accepted` + job id.
- Snapshots & clones: `POST /volumes/{id}/snapshots`, `POST /snapshots/{id}/clone|restore`,
  with parent/clone dependency tracking (protect against unsafe deletes).
- `atlas-policy`: map intent (`production|database|dev|archive|ai`) → pool/class/replication/
  encryption/snapshot/backup (PDF §12.3 default policies).
- **Idempotency keys** for all create operations (PDF §17.4).
